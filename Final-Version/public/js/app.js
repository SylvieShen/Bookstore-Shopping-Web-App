...
Store.prototype.checkOut = function(onFinish) {
    ...
    this.syncWithServer(function(delta) {
        ...
        if (alertMsg.length != 0) {
            ...
            var Order = {
                "client_id": Math.floor((Math.random() * 1000) + 1).toString(),
                "cart": currentStore.cart,
                "total": total
            };
            ajaxPost(store.serverUrl + "/checkout", Order,
                function(response) {
                    alert("All items are checked out successfully!");
                    currentStore.cart = {};
                    currentStore.onUpdate();
...
function renderCart(container, storeInstance) {
    ...
    for (itemName in storeInstance.cart) {
        ...
        itemNameshow.textContent = itemName;
